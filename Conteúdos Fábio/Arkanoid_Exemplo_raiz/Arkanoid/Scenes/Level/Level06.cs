...
namespace Arkanoid
{
    public class Level06 : BaseLevel
    {
            ...
            : base(game)
        {
            ...
            this.textureRect[2] = Game.Content.Load<Texture2D>(@"Images/verde");
            this.textureRect[3] = Game.Content.Load<Texture2D>(@"Images/vermelho");
            ...
            for (int j = 0; j < NUM_LINE; j++)
            {
                for (int i = 0; i < NUM_RECT; i++)
                {
                    Rect rect = new Rect(new Vector2(20 + i * this.textureRect[0].Width + i, j + 100 + (j * (this.textureRect[0].Height))),
                                         ref this.textureRect[random.Next(NUM_IMG)],
                                         Vector2.Zero,
                                         i * NUM_RECT + j);
